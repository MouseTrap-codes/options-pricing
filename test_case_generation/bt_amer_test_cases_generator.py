...
from .bt_amer_test_inputs import (
    AMERICAN_CALL_INPUTS,
    AMERICAN_EDGE_CASES,
    AMERICAN_PUT_INPUTS,
)
...
def binomial_american_price(
    ...
    *,
    ...
    exercise = ql.AmericanExercise(today, today + maturity_days)
...
def build_payload_binomial_amer() -> Dict[str, list]:
    calls = [make_bt_case(n, x) for n, x in AMERICAN_CALL_INPUTS]
    puts = [make_bt_case(n, x) for n, x in AMERICAN_PUT_INPUTS]
    edges = [make_bt_case(n, x) for n, x in AMERICAN_EDGE_CASES]
    return {
        "AMERICAN_CALLS": calls,
        "AMERICAN_PUTS": puts,
        "AMERICAN_EDGE_CASES": edges,
    }
...
if __name__ == "__main__":
    ...
    bt_payload = build_payload_binomial_amer()
